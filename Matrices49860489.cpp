...
void Hero::init(float x, float y)
{
	...
	l_x_pos = x_pos + (sin(alpha*DEGTORAD) * 50);
	l_y_pos = y_pos + (cos(alpha*DEGTORAD) * 50);
}
...
void Bullet::move()
{
 	x_pos = x_pos + sin(n_alpha*DEGTORAD)*10;
	y_pos = y_pos + cos(n_alpha*DEGTORAD)*10;
}
...
void do_game_logic(void)
{
	...
	if (bullet.show() == true)
	{
		...
		else if (bullet.y_pos > 430)
		{
			n_alpha = n_alpha - 90;
			bullet.move();
		}
		else if (bullet.x_pos > 480)
		{
			n_alpha = n_alpha - 90;
			bullet.move();
		}
...
void render_frame(void)
{
	...
	//런처
	RECT part3;
	SetRect(&part3, 0, 0, 32, 32);
	D3DXVECTOR3 center1(0.0f, 0.0f, 0.0f);    // center at the upper-left corner
	D3DXVECTOR3 position1(hero.x_pos+16 + (sin(alpha*DEGTORAD) * 50), hero.y_pos+16 + (cos(alpha*DEGTORAD) * 50), 0.0f);    // position at 50, 50 with no depth
	d3dspt->Draw(sprite_l_bullet, &part3, &center1, &position1, D3DCOLOR_ARGB(255, 255, 255, 255));
